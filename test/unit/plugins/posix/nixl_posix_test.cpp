...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "hn:s:d:w:m:")) != -1) {
        switch (opt) {
            ...
            case 'h':
            default:
                std::cout << absl::StrFormat("Usage: %s [-n num_transfers] [-s transfer_size] [-d test_files_dir_path] [-w wait_time] [-m max_waits]", argv[0]) << std::endl;
    ...
    nixlXferReqH      *treq;
    std::string       name;
    nixl_xfer_dlist_t dram_for_posix_list(DRAM_SEG);
    ...
    if (status != NIXL_SUCCESS) {
        std::cerr << absl::StrFormat("Error creating POSIX backend: %s", nixlEnumStrings::statusStr(status)) << std::endl;
        return 1;
    }
    ...
    dram_for_posix_list = dram_for_posix.trim();

    status = agent.createXferReq(NIXL_WRITE, dram_for_posix_list, file_for_posix_list,
                                 "POSIXTester", treq);
    if (status != NIXL_SUCCESS) {
    ...
    status = agent.createXferReq(NIXL_READ, dram_for_posix_list, file_for_posix_list,
                                 "POSIXTester", treq);
    if (status != NIXL_SUCCESS) {
